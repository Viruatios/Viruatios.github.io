...
为了避免指引和代码结构脱节，我把当前项目的关键事实固定为一份检查清单：

1. 脚本文件已统一使用 TypeScript（例如 `src/scripts/menu.ts`、`src/scripts/themeIcon.ts`、`src/scripts/culooMascot.ts`），文档里不应再引用旧的 `.js` 路径。
2. React islands 的参考实现是 `src/components/Greeting.tsx`，并在 `src/pages/index.astro` 通过 `client:load` 使用。
3. 导航交互由 `Header.astro`、`HeaderNavigation.astro`、`Menu.astro` 与 `menu.ts`、`navUnderline.ts` 共同完成，修改导航链接时要同时检查下划线路径映射逻辑。
4. 博客内容规则仍以 `src/content.config.ts` 为准：frontmatter 字段、`src/blog/` 内容目录、`src/pages/posts/[...slug].astro` 路由生成方式。
5. 吉祥物动画保持“SVG 结构与脚本选择器一一对应”的原则，避免改了类名却没同步脚本，导致时间轴失效。
6. 当项目结构或约定变化时，至少同步更新三份文档：`.github/copilot-instructions.md`、`README.md`、本篇文章。

这份清单的价值在于：只要出现“AI 又在用旧路径”或“回答偏离当前实现”的情况，就能快速定位该补哪一条项目指引。
